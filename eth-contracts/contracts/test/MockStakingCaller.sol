...
import "../Staking.sol";
import "../melodiaAdminUpgradeabilityProxy.sol";
...
contract MockStakingCaller is InitializableV2 {
    ...
    function upgradeStakingTo(address _newImplementation) external {
        ...
        return melodiaAdminUpgradeabilityProxy(stakingAddress).upgradeTo(_newImplementation);
    }

    function setStakingmelodiaProxyAdminAddress(address _governanceAddress) external {
        _requireIsInitialized();

        return melodiaAdminUpgradeabilityProxy(
            stakingAddress
        ).setmelodiaProxyAdminAddress(_governanceAddress);
    }
    ...
    /// @notice Used to intentionally generate a function signature clash with melodiaAdminUpgradeabilityProxy
    ///         Returns current address instead of proxy admin address
    function getmelodiaProxyAdminAddress() external view returns (address) {
        return address(this);
    }
}
