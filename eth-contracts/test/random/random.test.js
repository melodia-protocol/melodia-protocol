...
contract("Random testing", async (accounts) => {
  ...
  const printTestSummary = () => {
    console.log(
      `\n------------------------ melodia RANDOM TESTING Summary ------------------------`
    );
    console.log(
  ...
  beforeEach(async () => {
    ...
    token = await melodiaToken.at(process.env.tokenAddress);
    ...
    sysLog(
      `melodiaToken: ${token.address}, expected ${process.env.tokenAddress}`
    );
    sysLog(
  ...
  const initializeTestState = async () => {
    ...
    // https://github.com/melodiaProject/melodia-protocol/commit/12116eede803b395a9518c707360e7b633cf6ad2
  ...
  const delegate = async (serviceProvider, amount, delegator) => {
    try {
      ...
      if (amount.gt(currentBalance)) {
        let missing = amount.sub(currentBalance);
        await token.transfer(delegator, missing, {
...
outside=${info.outsideStake.toString()}=(deployerStake=${
  ...
  it("Random test suite", async () => {
    ...
    while (duration < TestDuration) {
      ...
      sysLog(
        `------------------------ melodia RANDOM TESTING - Round ${currentRound}, ${duration}/${TestDuration}ms ------------------------`
      );
      ...
      sysLog(
        `------------------------ melodia RANDOM TESTING - Finished Round ${currentRound} in ${roundDuration}ms ------------------------\n`
      );
      // Progress round
      currentRound++;
      // Update duration
    ...
    await logCurrentBlock();
    sysLog(
      `------------------------ melodia RANDOM TESTING SUMMARY - Finished ${currentRound} rounds in ${duration}ms ------------------------`
    );
