...
docker build \
    -t melodia/filebeat:${FILEBEAT_VERSION} \
...
docker build \
    -t melodia/metricbeat:${METRICBEAT_VERSION} \
    --build-arg git_sha=$(git rev-parse HEAD) \
    --build-arg ELASTIC_ENDPOINT=${ELASTIC_ENDPOINT} \
    --build-arg ELASTIC_CLOUD_ID=${ELASTIC_CLOUD_ID} \
    --build-arg API_ID=${API_ID} \
...
if [[ "${1}" == "push" ]]; then
    docker push melodia/metricbeat:${METRICBEAT_VERSION}
    docker push melodia/filebeat:${FILEBEAT_VERSION}
fi
